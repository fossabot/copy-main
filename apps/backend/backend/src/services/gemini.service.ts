<<<<<<< C:/Users/Mohmed Aimen Raed/Desktop/the...copy-main/backend/src/services/gemini.service.ts
...
/**
 * نوع فئات ذاكرة التخزين المؤقت المدعومة لـ Gemini
 * 
 * @description
 * يحدد الفئات الصالحة لمفاتيح ذاكرة التخزين المؤقت في Gemini API بناءً على البادئات المعرفة.
 * يساعد في تنظيم وتصنيف البيانات المخزنة مؤقتاً لسهولة الاسترجاع والإدارة.
 */
type GeminiCacheCategory = keyof typeof GEMINI_CACHE_PREFIX;

/**
 * إعدادات تكوين طلب Gemini API
 * 
 * @description
 * يحدد كافة المعاملات المطلوبة لتنفيذ طلب Gemini مع دعم كامل للتخزين المؤقت، 
 * الحماية (Guardrails)، والقياسات (Metrics).
 */
interface GeminiRequestConfig {
  /** 
   * معرّف نوع الطلب.
   * يستخدم في عمليات التخزين المؤقت، تسجيل الأخطاء، وتتبع مقاييس الأداء.
   */
  requestType: string;
  
  /** 
   * النص الموجه (Prompt) النهائي المرسل إلى النموذج.
   * يجب أن يكون مصاغاً بدقة للحصول على أفضل النتائج من Gemini.
   */
  prompt: string;
  
  /** 
   * النص المدخل الأصلي من قبل المستخدم.
   * ضروري لعمليات التحقق من الحماية (Guardrails) قبل المعالجة.
   */
  originalInput: string;
  
  /** 
   * معاملات إضافية لتوليد مفتاح ذاكرة التخزين المؤقت الفريد.
   */
  cacheKeyParams: Record<string, unknown>;
  
  /** 
   * فئة التخزين المؤقت التي ينتمي إليها الطلب (مثلاً: 'character', 'analysis').
   */
  cacheCategory: GeminiCacheCategory;
  
  /** 
   * خيار لاستخدام وقت انتهاء صلاحية (TTL) تكيفي بناءً على معدل نجاح الوصول للذاكرة.
   */
  useAdaptiveTTL?: boolean;
  
  /** 
   * رسالة الخطأ المخصصة التي تظهر للمستخدم عند فشل العملية (بالعربية).
   */
  errorMessage: string;
}

/**
 * خدمة Gemini للهندسة اللغوية والذكاء الاصطناعي
 * 
 * @description
 * الفئة المركزية المسؤولة عن التواصل مع نماذج Google Gemini.
 * تتولى إدارة دورة حياة الطلب بالكامل بما في ذلك:
 * 1. المصادقة والتحقق من مفاتيح API.
 * 2. تطبيق قواعد الحماية (LLM Guardrails) على المدخلات والمخرجات.
 * 3. إدارة التخزين المؤقت الذكي (Caching Strategy).
 * 4. تتبع استهلاك التوكنات والتكاليف المالية (Cost Tracking).
 * 5. توفير مقاييس الأداء (Performance Metrics).
 * 
 * @architecture Singleton / Service Layer
 */
export class GeminiService {
  private genAI: GoogleGenerativeAI;
  private model: any;
  private readonly REQUEST_TIMEOUT = 30000; // 30 ثانية كحد أقصى للطلب
  ...
  /**
   * تتبع استهلاك التوكنات وحساب التكاليف
   * 
   * @description
   * يستخرج بيانات الاستهلاك من استجابة API ويقوم بتحديث نظام تتبع التكاليف.
   * 
   * @param {any} apiResult - الاستجابة الخام من Gemini API
   * @param {string} analysisType - نوع التحليل الذي تم إجراؤه
   * @private
   */
  private async trackTokenUsage(apiResult: any, analysisType: string): Promise<void> {
    try {
  ...
  /**
   * تطبيق نظام الحماية والقيود (Guardrails)
   * 
   * @description
   * يقوم بفحص المدخلات والمخرجات لضمان توافقها مع سياسات الأمان والمحتوى.
   * يمنع الهجمات مثل Prompt Injection ويصفي المحتوى غير اللائق.
   * 
   * @param {string} input - النص المراد فحصه
   * @param {string} output - النص المولد المراد فحصه
   * @param {string} requestType - نوع الطلب لتطبيق قواعد مخصصة
   * @param {string} [userId] - معرف المستخدم (اختياري)
   * @returns {{ sanitizedInput: string; sanitizedOutput: string; warnings: string[] }}
   * @throws {Error} في حال اكتشاف انتهاك صريح للسياسات
   * @private
   */
  private applyGuardrails(
    input: string,
  ...
  /**
   * المحرك المركزي لتنفيذ طلبات Gemini
   * 
   * @description
   * يقوم بتجميع كافة الخطوات المعقدة للطلب في مكان واحد:
   * 1. توليد مفتاح التخزين المؤقت.
   * 2. تطبيق نظام Stale-While-Revalidate للاستجابات السريعة.
   * 3. معالجة زمن انتهاء الطلب (Timeout).
   * 4. تطبيق الحماية (Guardrails).
   * 5. تتبع القياسات والتكاليف.
   * 
   * @param {GeminiRequestConfig} config - إعدادات الطلب
   * @returns {Promise<string>} النص المولد والمنقح
   * @throws {Error} في حال فشل الطلب أو انتهاك الحماية
   * @private
   */
  private async executeGeminiRequest(config: GeminiRequestConfig): Promise<string> {
    const { requestType, prompt, originalInput, cacheKeyParams, cacheCategory, useAdaptiveTTL, errorMessage } = config;
  ...
  /**
   * تحليل نص درامي بناءً على نوع محدد
   * 
   * @param {string} text - النص المراد تحليله
   * @param {string} analysisType - نوع التحليل (مثلاً: 'characters', 'themes')
   * @returns {Promise<string>} تقرير التحليل المولد
   * @example
   * await geminiService.analyzeText("نص المشهد...", "characters");
   */
  async analyzeText(text: string, analysisType: string): Promise<string> {
    const validCategories: Record<string, GeminiCacheCategory> = {
      'characters': 'character',
  ...
  /**
   * توليد محتوى عام من Prompt
   * 
   * @description
   * غلاف بسيط حول نموذج Gemini لتوليد محتوى دون معالجة معقدة.
   * 
   * @param {string} prompt - النص الموجه
   * @param {{ temperature?: number; maxTokens?: number }} [options] - خيارات التوليد
   * @returns {Promise<string>} المحتوى المولد
   */
  async generateContent(prompt: string, options?: { temperature?: number; maxTokens?: number }): Promise<string> {
    try {
      const result = await this.model.generateContent(prompt);
  ...
  /**
   * مراجعة سيناريو درامي بشكل احترافي
   * 
   * @param {string} text - نص السيناريو
   * @returns {Promise<string>} تقرير المراجعة والنقد
   */
  async reviewScreenplay(text: string): Promise<string> {
    const prompt = `أنت خبير في كتابة السيناريوهات العربية. قم بمراجعة النص التالي وقدم ملاحظات على:
1. استمرارية الحبكة
...
${text}`;
   ...
   * @param {any} [context] - السياق الإضافي للمحادثة
...
السؤال: ${message}`;
  ...
  /**
   * الحصول على اقتراحات لقطات سينمائية
   * 
   * @param {string} sceneDescription - وصف المشهد
   * @param {string} shotType - نوع اللقطة المطلوب
   * @returns {Promise<string>} تفاصيل اللقطة المقترحة
   */
  async getShotSuggestion(sceneDescription: string, shotType: string): Promise<string> {
    const prompt = `أنت خبير في إخراج الأفلام العربية. قدم اقتراحًا مفصلًا لنوع اللقطة "${shotType}" للمشهد التالي:
...
5. المدة التقديرية`;
  ...
  /**
   * بناء الـ Prompt المناسب بناءً على نوع التحليل
   * 
   * @param {string} text - النص المدخل
   * @param {string} analysisType - نوع التحليل
   * @returns {string} الـ Prompt المصاغ
   * @private
   */
  private buildPrompt(text: string, analysisType: string): string {
    const prompts = {
...
/**
 * تصدير نسخة وحيدة (Singleton) من خدمة Gemini
 */
export const geminiService = new GeminiService();
>>>>>>> C:/Users/Mohmed Aimen Raed/.windsurf/worktrees/the...copy-main/the...copy-main-83821a91/backend/src/services/gemini.service.ts
